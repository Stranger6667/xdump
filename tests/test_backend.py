...
@pytest.mark.parametrize(
    ...
    (
        ("", b"id,name\n1,Admin\n2,User\n"),
        (
            "INSERT INTO groups (id, name) VALUES (3, 'test')",
            b"id,name\n1,Admin\n2,User\n3,test\n",
        ),
    ),
)
...
class TestHighLevelInterface:
    ...
    def test_load(self, backend, archive_filename, db_helper):
        ...
        assert backend.run("SELECT name FROM groups") == [
            {"name": "Admin"},
            {"name": "User"},
        ]
        if IS_POSTGRES:
...
class TestAutoSelect:
    ...
    @pytest.mark.dump(
        [],
        {
            "tickets": "SELECT * FROM tickets WHERE id = 1",
            "employees": "SELECT * FROM employees WHERE id = 2",
        },
    )
    def test_multiple_partials(self):
        # When different entries from ``partial_tables`` have references to the same relation
        # Then output should contain all data required for all mentioned entries without doubling.
        self.assert_content("tickets", {TICKETS_HEADER, b"1,1,Sub 1,Message 1"})
        self.assert_content("employees", {EMPLOYEES_HEADER, BLACK, DOE})
        self.assert_group()
    ...
    @pytest.mark.dump(
        [],
        {
            "tickets": "SELECT * FROM tickets WHERE id = 3",
            "employees": "SELECT * FROM employees WHERE id = 5",
        },
    )
