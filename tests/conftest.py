...
def is_search_path_fixed(connection):
    ...
    return (
        version >= 100003
        or 90608 <= version < 100000
        or 90512 <= version < 90600
        or 90417 <= version < 90500
...
class BackendWrapper(object):
    ...
    def concurrent_insert(self, query):
        ...
        with patch.object(
            self.backend,
            "export_to_csv",
            wraps=self.backend.export_to_csv,
...
class PostgreSQLWrapper(BackendWrapper):
    ...
    def is_database_exists(self, dbname):
        return self.backend.run("SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = %s)", [dbname])[0]["exists"]

    def assert_namelist(self, archive):
        assert archive.namelist() == [
            "dump/schema.sql",
            "dump/sequences.sql",
            "dump/data/groups.csv",
    ...
    def get_tables_count(self):
        return self.backend.run("SELECT COUNT(*) FROM pg_tables WHERE tablename IN ('groups', 'employees', 'tickets')")[
            0
        ]["count"]
...
def db_helper(request, backend):
    return {"postgres": PostgreSQLWrapper, "sqlite": SQLiteWrapper,}[
        DATABASE
